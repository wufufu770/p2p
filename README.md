...
**归档原因**：同一套「三环并行渗透测试」架构（graphd 共享状态 + 发现/深度/创造三环 + scope 门控仲裁），实测在 **dsh 宿主上的 d2d 实现效率高于 pi 宿主上的 p2p**：dsh + d2d 功能更多、使用更方便，日常使用与全部开发均已迁至 **[wufufu770/d2d](https://github.com/wufufu770/d2d)**。

**演进脉络**：
1. `p2p`（本仓，2026-08-22）— pi agent 插件单体，首套三环并行渗透架构
2. `p2p-core`（2026-08-24）— 宿主无关内核抽取，本仓改为调用内核的薄壳
3. **`d2d`（2026-09-02 起，现役）** — dsh 插件，当前唯一活跃开发线

本仓保留为历史参考。issue / PR 不再受理；如需使用或继续开发，请前往 d2d 仓库。
